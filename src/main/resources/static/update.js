...
const name_ed = document.getElementById('First name_ed');
const last_name_ed = document.getElementById('Last name_ed');
    ...
    if (usersPageEd.ok) {
        let userData =
            await usersPageEd.json().then(async user => {
                ...
                last_name_ed.value = `${user.lastName}`;
...
async function editUser() {
    let urlEdit = '/rest/admin/' + id_ed.value
    let listOfRole = [];
